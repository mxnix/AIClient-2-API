...
- каждые 30 минут проверяет все Gemini fixed IP;
- публикует и редактирует один пост в Telegram-канале;
- добавляет под пост inline-кнопку `Обновить` для досрочной ручной проверки;
...
под постом отображается кнопка `Обновить`.

при нажатии бот запускает внеочередную проверку, но с защитой:

- только одна проверка может идти одновременно;
- есть глобальный cooldown после любой завершенной проверки;
- есть отдельный короткий cooldown на конкретного пользователя.

env-параметры:

- `MANUAL_REFRESH_ENABLED=true`
- `MANUAL_REFRESH_GLOBAL_COOLDOWN_MS=300000`
- `MANUAL_REFRESH_USER_COOLDOWN_MS=60000`
- `TELEGRAM_GET_UPDATES_TIMEOUT_SEC=30`
- `TELEGRAM_GET_UPDATES_ERROR_RETRY_MS=5000`

важно: для кнопки используется `getUpdates` long polling. у этого бота не должен быть активен webhook на том же токене.
